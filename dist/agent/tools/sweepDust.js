...
async function sweepDust(walletAddress, mode, maxTokens = 20) {
    try {
        const response = await axios_1.default.post(HELIUS_RPC, {
            jsonrpc: "2.0",
            id: "sweep",
        ...
        const zeroAccounts = allAccounts
            .filter((acc) => (acc.account?.data?.parsed?.info?.tokenAmount?.uiAmount ?? 1) === 0)
            .slice(0, maxTokens);
