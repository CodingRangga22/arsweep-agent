...
async function getTokenName(mint: string): Promise<{ symbol: string; name: string }> {
  try {
...
async function analyzeToken(mint: string): Promise<any> {
  const signals: any[] = [];
  ...
  try {
    ...
    const mintInfoRes = await axios.post(HELIUS_RPC, {
    ...
    const scoreMap: Record<string, number> = { low: 10, medium: 25, high: 40, critical: 60 };
...
export async function checkScam(walletAddress: string, specificMint?: string): Promise<string> {
  try {
    let mints: string[] = [];
