...
// ── 1: Analyze Wallet ─────────────────────────────────────────────────────
export async function analyzeWalletFree(req: Request, res: Response) {
  try {
...
// ── 2: Sweep Report ───────────────────────────────────────────────────────
export async function sweepReportFree(req: Request, res: Response) {
  try {
    const { walletAddress } = req.body;
    if (!walletAddress) return res.status(400).json({ error: "walletAddress required" });
...
// ── 3: Wallet Roast ───────────────────────────────────────────────────────
export async function walletRoastFree(req: Request, res: Response) {
  try {
    const { walletAddress } = req.body;
    if (!walletAddress) return res.status(400).json({ error: "walletAddress required" });
    ...
    let score = 100;
    if (solSOL < 0.01) score -= 30; else if (solSOL < 0.1) score -= 15;
    if (emptyRatio > 0.7) score -= 30; else if (emptyRatio > 0.4) score -= 15;
...
// ── 4: Rug Pull Detector ──────────────────────────────────────────────────
export async function rugPullDetectorFree(req: Request, res: Response) {
  try {
    const { walletAddress } = req.body;
    if (!walletAddress) return res.status(400).json({ error: "walletAddress required" });
    ...
    const results = await Promise.allSettled(tokens.slice(0, 10).map(async (token) => {
      ...
      } catch { return { mint: token.mint, balance: token.balance, score: 0, risks: [], name: "Unknown", symbol: "???", verdict: "UNVERIFIED" }; }
    }));
    const analyzed = results.filter(r => r.status === "fulfilled").map(r => (r as any).value);
...
export async function autoSweepPlannerFree(req: Request, res: Response) {
  try {
    const { walletAddress } = req.body;
    if (!walletAddress) return res.status(400).json({ error: "walletAddress required" });
    ...
    const solSOL = solBalance / LAMPORTS_PER_SOL;
    const RENT = 0.00203928; const FEE = 0.015;
